...
def TwoTower(user_dnn_feature_columns, item_dnn_feature_columns, bottom_dnn_hidden_units=(256, 128), tower_dnn_hidden_units=(64,),
    ...
    features = build_input_features(user_dnn_feature_columns + item_dnn_feature_columns)

    inputs_list = list(features.values())

    user_sparse_embedding_list, user_dense_value_list = input_from_feature_columns(features, user_dnn_feature_columns, l2_reg_embedding, seed)
    item_sparse_embedding_list, item_dense_value_list = input_from_feature_columns(features, item_dnn_feature_columns, l2_reg_embedding, seed)

    user_dnn_input = combined_dnn_input(user_sparse_embedding_list, user_dense_value_list)
    item_dnn_input = combined_dnn_input(item_sparse_embedding_list, item_dense_value_list)

    user_bottom_output = DNN(bottom_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed)(user_dnn_input)
    item_bottom_output = DNN(bottom_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed)(item_dnn_input)

    tasks_output = []
    for task_type, task_name in zip(task_types, task_names):
        user_tower_output = DNN(tower_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                           name='user_tower_' + task_name)(user_bottom_output)
        item_tower_output = DNN(tower_dnn_hidden_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                           name='item_tower_' + task_name)(item_bottom_output)
        logit = Dense(1, use_bias=False)(Multiply()([user_tower_output, item_tower_output]))
